library(tidyverse)
interest <- read_csv("/Users/renatorusso/Desktop/Ed.D./HUDK-4051/interest.csv")
difficulty <- read_csv("/Users/renatorusso/Desktop/Ed.D./HUDK-4051/difficulty.csv")
View(interest)
View(difficulty)
...
# Then, I'll transpose the dataframe (i.e., columns become rows, rows become columns)
#library(data.table)
interest_means_t <- transpose(interest_means)
difficulty_means_t <- transpose(difficulty_means)
...
# Pllotting metadata (inteerest and difficulty)
p <- ggplot(data = difficulty_interest, aes(x = difficulty,
                                     y = interest,
